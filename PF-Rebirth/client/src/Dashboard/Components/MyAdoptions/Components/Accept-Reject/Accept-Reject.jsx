...
console.log()
  ...
  return (
    <>
    {  adoptChat.filter((adChat)=>adChat.id===adoptionId).map((datos)=>{
    if (datos.owner.mail===mail){
    return <div className="mainDashCont">
        <div className="AdoptContainer">
        <div >
          <div class="imgFav">
            <img  src={datos.adopter.image} alt="Adopter" class="img" />
          </div>
        </div>
        <div >
          <span>Name: {datos.adopter.name} {datos.adopter.lastName}</span>
          <br />
